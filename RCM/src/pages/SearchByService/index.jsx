...
const SearchByService = () => {
    ...
    const onProviderNameChange = (value) => {
        if (value) {
            setSelectedProviderName(value);
            const filtered = tableData.filter(item => item.provider_last_org_name === value);
    ...
    const onProviderCityChange = (value) => {
        if (value && selectedProviderName) {
            const filtered = tableData.filter(item => item.provider_last_org_name === selectedProviderName && item.provider_city === value);
            setFilteredData(filtered);
        } else {
            setFilteredData(tableData);
        }
                    ...
                    <div className='p-4 flex  ' style={{ border: "1px solid grey ", borderRadius: "4px", width: "58%" }}>
                        <div style={{ width: "100%" }}>
                            <Space.Compact size="large" style={{ width: "100%" }}>
                                <Input fullWidth addonBefore={<SearchOutlined />} placeholder="Services" onChange={(e) => setServices({ "user_req": e.target.value })} />
                            </Space.Compact>
                        </div>
